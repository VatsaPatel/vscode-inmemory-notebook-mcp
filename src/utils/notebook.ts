...
export async function resolveNotebook(notebookUri?: string, options: { allowSingleNotebookFallback: boolean } = { allowSingleNotebookFallback: false }): Promise<NotebookAccessResult> {
  ...
  if (!options.allowSingleNotebookFallback) {
    return {
      allowed: false,
      error: "No notebook target is pinned. Connect through a notebook session URL or pass notebook_uri explicitly."
    };
  }
...
export async function checkCanModifyNotebook(notebookUri?: string): Promise<NotebookAccessResult> {
  return await resolveNotebook(notebookUri, { allowSingleNotebookFallback: false });
}
...
export async function checkCanReadNotebook(notebookUri?: string): Promise<NotebookAccessResult> {
  return await resolveNotebook(notebookUri, { allowSingleNotebookFallback: true });
}
