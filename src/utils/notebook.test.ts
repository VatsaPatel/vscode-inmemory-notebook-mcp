...
describe('editCellContent (logic verification)', () => {
  it('should be exported from notebook module', async () => {
    const notebook = await import('./notebook.js');
...
describe('checkCanModifyNotebook', () => {
  ...
  it('requires an explicit target when no notebooks are open', async () => {
    (vscode.window as any).activeNotebookEditor = undefined;
    (vscode.workspace as any).notebookDocuments = [];
    ...
    expect(result.error).toContain('No notebook target is pinned');
  });

  it('does not use the active editor as an implicit write target', async () => {
  ...
  it('does not use the focused editor as an implicit write target', async () => {
    const mockNotebook = { uri: { path: '/test.ipynb' } };
    const mockEditor = { notebook: mockNotebook };
    (vscode.window as any).activeNotebookEditor = mockEditor;
    (vscode.window as any).state = { focused: true };
  ...
  it('does not use single open notebook fallback for writes', async () => {
    const mockNotebook = {
  ...
  it('returns missing-target before considering multiple open notebooks for writes', async () => {
    const mockNotebook1 = {
...
describe('checkCanReadNotebook', () => {
  ...
  it('does not use the active editor as an implicit read target', async () => {
    const mockNotebook = { uri: { path: '/test.ipynb' } };
    const mockEditor = { notebook: mockNotebook };
    (vscode.window as any).activeNotebookEditor = mockEditor;
    (vscode.window as any).state = { focused: false };
  ...
  it('ignores the active editor when no open notebook document is registered', async () => {
    const mockNotebook = { uri: { path: '/test.ipynb' } };
    const mockEditor = { notebook: mockNotebook };
    (vscode.window as any).activeNotebookEditor = mockEditor;
    (vscode.window as any).state = { focused: true };
